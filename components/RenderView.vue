<template>
    <section>
        <div v-if="pending">Loading</div>
        <div v-else class="container">
            <h1 class="text-4xl text-center">{{ getViewByRoute?.title }}</h1>
            <markdown v-if="getViewByRoute?.content != null" :source="getViewByRoute?.content" />
            <!--<button v-if="getIsAdmin"
                class="text-white bg-primary font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2" type="submit"
                @click="edit">
                編輯
            </button>-->
        </div>
    </section>
</template>

<script setup lang="ts">
import { Markdown } from "#components";
import { GET_VIEW_BY_ROUTE } from "~/apollo/view";

const props = defineProps({
    route: String
})
const { data, pending } = await useLazyAsyncQuery(GET_VIEW_BY_ROUTE, { route: props.route });
const getViewByRoute = data.value?.getViewByRoute;

// const view =
// console.log(data.value)
// const { data } = await useFetch(`/api/views/${props.route}`);
// console.log("data", data?.value)
// const view = data?.value?.getViewByRoute;
// const { data } = await useAsyncGql('getViewByRouteQuery', { route: props.route });

// console.log((view))

// const { toggleModel, updateView } = store;

// const { getIsAdmin } = storeToRefs(store);

// function edit() {
//     const modelDetails = {
//         content: view.content,
//         submitFunction: submitChange,
//     };
//     toggleModel(modelDetails);
// }

// function submitChange(updatedContent) {
//     updateView({
//         viewId: view.id,
//         content: updatedContent,
//     });
// }

</script>

