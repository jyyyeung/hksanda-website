<template>
  <section class="z-10">
    <header class="z-30 grid h-auto grid-cols-3 gap-0 md:grid-cols-6 lg:grid-cols-12 lg:grid-rows-1 lg:gap-3">
      <NuxtLink to="/" class="nav-link col-span-1 col-start-1 row-start-1">
        <nuxt-img width="72" alt="logo" class="mx-2 my-1" src="/images/logo.png" sizes="sm:48 md:52 lg:71" />
      </NuxtLink>
      <nav class="col-span-6 row-start-2 lg:col-span-9 lg:row-start-1 lg:my-1 lg:h-auto lg:self-center">
        <ul class="flex flex-wrap lg:min-h-[3em] lg:flex-row lg:items-center lg:gap-3">
          <li class="mx-2 min-h-[48px] w-fit md:min-h-min" v-for="route in (routes as Route[])">
            <NuxtLink :href="route.to" :class="`nav-link group block px-2 text-center text-text-color hover:bg-transparent hover:bg-brush hover:bg-cover hover:bg-center hover:text-white  ${route.to == currentRoute.path
